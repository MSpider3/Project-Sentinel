...
# Handle the mehul -> mehulgolecha rename if it exists
if [ -f "models/gallery_mehul.npy" ]; then
    info "Renaming gallery_mehul.npy to gallery_mehulgolecha.npy..."
    mv models/gallery_mehul.npy models/gallery_mehulgolecha.npy
fi
cp -r models/. "$PROJECT_LIB/models/"
success "AI models and galleries installed."

cd "$PROJECT_LIB"
info "Building secure Python environment (.venv)..."
# Create a fresh venv
"$UV_BIN" venv "$PROJECT_LIB/.venv" --python python3 >> "$LOG_FILE" 2>&1 || {
    warn "uv venv failed. Retrying with system python..."
    python3 -m venv "$PROJECT_LIB/.venv" >> "$LOG_FILE" 2>&1
}

# Copy the modern pyproject.toml to the installation root
cp "$CURRENT_DIR/pyproject.toml" "$PROJECT_LIB/pyproject.toml"

# Use UV PIP to force-install all core AI dependencies into that venv
# This is more reliable than 'uv sync' in non-standard system directories.
info "Synchronizing AI runtime dependencies..."
"$UV_BIN" pip install --python "$PROJECT_LIB/.venv" \
    "textual==0.86.0" \
    "rich==13.7.0" \
    "opencv-python>=4.9.0" \
    "numpy>=1.26.0" \
    ...
    "scipy>=1.11.0" \
    "mediapipe>=0.10.11" \
    "python-pam>=2.0.2" >> "$LOG_FILE" 2>&1 || {
    error_msg "Dependency installation failed. Check $LOG_FILE."
    exit 1
}

cd "$CURRENT_DIR"
success "Python environment (.venv) is ready."

# ── Step 8: Global 'sentinel' Command & PAM Module ───────────────────────────
info "Compiling native PAM module..."
[[ -f "/lib64/security/pam_sentinel.so" ]] && rm -f /lib64/security/pam_sentinel.so
gcc -fPIC -shared -D_GNU_SOURCE "$CURRENT_DIR/core/pam_sentinel.c" -o /lib64/security/pam_sentinel.so -lpam 2>> "$LOG_FILE" || {
    warn "Native PAM module compilation failed! Falling back to python client."
}

info "Installing global 'sentinel' command (secured)..."
cat > /usr/bin/sentinel << 'EOF'
#!/bin/bash
if [[ $EUID -ne 0 ]]; then
   echo "Error: Sentinel TUI requires root privileges."
   echo "Please run with: sudo sentinel"
   exit 1
fi
export SENTINEL_SOCKET_PATH=/run/sentinel/sentinel.sock
cd /usr/lib/project-sentinel
PYTHON_EXEC="/usr/lib/project-sentinel/.venv/bin/python3"
case "$1" in
    preview) shift; "$PYTHON_EXEC" sentinel_tui/scripts/camera_preview.py "$@" ;;
    daemon)  shift; "$PYTHON_EXEC" sentinel_service.py "$@" ;;
    *)       "$PYTHON_EXEC" -m sentinel_tui "$@" ;;
esac
EOF
chmod +x /usr/bin/sentinel
success "Global 'sentinel' command installed (root-only)."
...
if [ -n "$REAL_USER" ]; then
    ...
    usermod -aG audio "$REAL_USER" 2>/dev/null && \
        success "Added $REAL_USER to 'audio' group (re-login to activate)." || true
fi

# Deploy audio assets for instruction playback
info "Installing native audio assets..."
mkdir -p "$PROJECT_LIB/audio/en"
if [ -d "$CURRENT_DIR/tools/audio_files" ]; then
    cp -r "$CURRENT_DIR/tools/audio_files/"* "$PROJECT_LIB/audio/en/" 2>/dev/null || true
fi
chmod -R 644 "$PROJECT_LIB/audio/en/"* 2>/dev/null || true
if ls "$PROJECT_LIB/audio/en"/*.wav 1>/dev/null 2>&1; then
    success "Audio assets deployed to $PROJECT_LIB/audio/en"
else
    warn "Audio assets missing; audio prompts may be silent"
fi
...
# Ensure /run/sentinel directory exists with proper permissions (systemd RuntimeDirectory may delay)
mkdir -p /run/sentinel
chmod 0755 /run/sentinel
chown root:root /run/sentinel

# Clean any stale Python cache to prevent module conflicts
find "$PROJECT_LIB" -type d -name __pycache__ -exec rm -rf {} + 2>/dev/null || true

systemctl restart sentinel-backend
success "sentinel-backend service started."

# ── Step 13: Persistent Camera Permissions (udev rule) ─────────────────────
info "Installing udev rule for persistent camera permissions..."
cat > /etc/udev/rules.d/99-sentinel-camera.rules << 'UDEV_EOF'
# Ensure camera devices are accessible to 'video' group on every boot
SUBSYSTEM=="video4linux", MODE="0660", GROUP="video"
SUBSYSTEM=="usb", ATTRS{idVendor}=="*", MODE="0666"
UDEV_EOF
udevadm control --reload-rules 2>/dev/null || true
udevadm trigger /dev/video* 2>/dev/null || true
success "Camera udev rules installed (persistent across reboots)."

# ── Step 14: Runtime Verification ────────────────────────────────────────────
info "Verifying daemon initialization..."
# Wait up to 10 seconds for the daemon to start and initialize
for i in {1..20}; do
    if systemctl is-active --quiet sentinel-backend 2>/dev/null; then
        # Daemon is running — wait for socket to appear
        if [ -S /run/sentinel/sentinel.sock ]; then
            success "Daemon socket found at /run/sentinel/sentinel.sock"
            break
        fi
    fi
    sleep 0.5
...
# Test socket connectivity with a simple ping
info "Testing daemon connectivity..."
...
try:
    ...
    sock.settimeout(2)
    sock.connect('/run/sentinel/sentinel.sock')
    ...
    if response.get("result", {}).get("success"):
        ...
        print("✗ Daemon responded but with error")
        exit(1)
except Exception as e:
    print(f"✗ Daemon connectivity test failed: {e}")
    exit(1)
finally:
    try:
        sock.close()
...
if [ $? -eq 0 ]; then
    success "Daemon connectivity verified."
else
    warn "Daemon connectivity test failed. Check: sudo systemctl status sentinel-backend"
fi
...
echo ""
echo -e "${YELLOW}IMPORTANT: For full functionality, please perform these steps:${NC}"
echo "  1. ${YELLOW}Re-login to your desktop session${NC} (log out and back in)"
echo "     → Activates 'video' group membership for camera access"
echo "  2. ${YELLOW}(Optional) Reboot your system${NC}"
echo "     → Ensures all permissions persist and udev rules are loaded"
echo ""
echo "    sudo reboot"
echo ""
echo -e "${YELLOW}Troubleshooting:${NC}"
echo "  • If camera doesn't open after re-login: log out/in again"
echo "  • If daemon fails: check sudo journalctl -u sentinel-backend"
echo "  • If PAM/sudo doesn't work: verify /etc/pam.d/sudo has 'pam_sentinel.so' rule"
echo ""
echo -e "${YELLOW}==========================================${NC}"
echo "To enable Face Unlock for GDM / sudo, add this to the TOP of the 'auth' section"
echo "in /etc/pam.d/gdm-password and /etc/pam.d/sudo:"
echo ""
echo "  auth sufficient pam_sentinel.so"
echo ""
warn "Warning: If pam_sentinel.so didn't compile, use the python fallback:"
